...
def test_command_center_no_invented_numbers(container):
    ...
    assert snap["version"] == "0.25.0"  # iteración 026 (Owner Command Center)
    assert snap["iteration"] == "026"
...
def test_frontend_backend_version_synced(tmp_path):
    ...
    assert 'data-wawa-version="0.25.0"' in index
    assert 'data-iteration="026"' in index
    assert 'data-build="026-owner-command-center"' in index
    ...
    settings = Settings()
    assert settings.version == "0.25.0"
