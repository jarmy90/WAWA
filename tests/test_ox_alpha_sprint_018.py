...
def test_command_center_no_invented_numbers(container):
    ...
    assert snap["version"] == "0.24.0"  # iteración 023 (recuperación del comité)
...
def test_frontend_backend_version_synced(tmp_path):
    ...
    assert 'data-wawa-version="0.24.0"' in index
    assert 'data-iteration="025"' in index
    assert 'data-build="025-autonomous-24-7"' in index
    ...
    settings = Settings()
    assert settings.version == "0.24.0"
