...
def test_command_center_no_invented_numbers(container):
    ...
    assert snap["version"] == "0.20.0"  # iteración 021 (activación comercial)
    assert snap["iteration"] == "021"
...
def test_frontend_backend_version_synced(tmp_path):
    ...
    assert 'data-wawa-version="0.20.0"' in index
    assert 'data-iteration="021"' in index
    assert 'data-build="021-commercial-activation"' in index
    ...
    settings = Settings()
    assert settings.version == "0.20.0"
