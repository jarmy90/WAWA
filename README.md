...
**MVP v0.11.1 (iteración 012)** — funciona 100% offline, sin APIs obligatorias. Gemini/OpenRouter/OmniRoute son opcionales. Al abrir `http://127.0.0.1:8000` se ve, **sin desplazamiento y sin necesidad de JavaScript**, la portada: WAWA, versión, **PRE_CYCLE · reloj detenido · 30 días**, **PRIMERA CAMPAÑA REAL 001**, el botón **INICIAR CAMPAÑA REAL** y la navegación (Inicio, Campaña real, Ideas, Investigación, Comité, Experimento, Economía, Actividad, Configuración) — la navegación y el botón viven en el HTML estático, no los crea JS. El panel antiguo (filtros, motor, economía, actividad, proveedor) quedó en sus sub-vistas. Diagnóstico visible con versiones de backend/frontend y estado de JavaScript; franja roja si el init falla. Incluye dashboard web local en `/` (una sola interfaz operativa: Campaña real, Ideas, Laboratorio, Comité, Economía), **orquestador end-to-end** (descubrimiento → filtros → torneo → investigación → evidencias → finalistas → comité → decisión → plan de experimento → **PRE_CYCLE** con reloj de 30 días que solo arranca con activación deliberada del propietario), **Business Discovery Engine** (campañas de descubrimiento abierto, General AI Substitution Test, Venture Quality Score, torneo de ideas, misiones Freebuff-first), comité externo visual (copiar/pegar en GPT/Grok/Gemini con decisión autónoma determinista), scoring determinista, BudgetGuard, modos de operación (con `PRODUCTION_ARMED` y arranque seguro → `SAFE_PAUSE`), **economía simulada auditada** (ledger append-only, idempotencia, reversiones, métricas, reconciliación) y **283 tests**. La entrega es verificable por construcción: frontend **sin caché** (`Cache-Control: no-store`), assets versionados (`?v=011`), marcador de versión visible con autochequeo contra `/api/health` (banner si el navegador sirve una copia antigua), sin botón de demo MQL5, y franja de estado siempre visible con PRE_CYCLE, campaña y próxima acción.
